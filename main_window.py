...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.logger.info("Initialize the main window")
        
        try:
            # Initialize the vector storage and do not reset the data directory
            ...
            # If there is no collection, create a default collection
            collections = self.store.get_collections()
            if not collections:
                default_collection = "Default knowledge base"
                if self.store.create_collection(default_collection):
                    self.store.current_collection = default_collection
                    self.refresh_kb_list()
                    self.logger.info(f"The default collection has been created: {default_collection}")
                else:
                    self.logger.error("Failed to create the default collection")
                    QMessageBox.warning(self, "Warning", "Failed to create the default collection. Please create the knowledge base manually")
            ...
            msg.setText("Unable to connect to the Qdrant server")
            ...
            msg.setText("The application initialization failed")
            msg.setInformativeText(str(e))
            msg.setStandardButtons(QMessageBox.StandardButton.Ok)
            msg.exec()
            self.close()
    ...
    def refresh_kb_list(self):
        ...
        try:
            self.logger.info("Refresh the knowledge base list")
            collections = self.store.get_collections()
            ...
            self.logger.error(f"Failed to refresh the knowledge base list: {str(e)}")
            QMessageBox.critical(self, "Error", f"Failed to refresh the knowledge base list: {str(e)}")

    def refresh_search_module(self):
        """Refresh the search module"""
        try:
            # Update the knowledge base selection drop-down box
            self.kb_select.clear()
            self.kb_select.addItems(self.store.get_collections())
            
            # If there is query content in the search box, re-execute the search
            current_query = self.search_input.text()
            if current_query:
                self.search()
        except Exception as e:
            self.logger.error(f"Failed to refresh the search module: {str(e)}")
    ...
    def delete_kb(self, name):
        ...
        try:
            reply = QMessageBox.question(
                self,
                "Confirm deletion",
                f"Are you sure you want to delete the knowledge base {name}?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            
            if reply == QMessageBox.StandardButton.Yes:
                self.logger.info(f"Delete the knowledge base: {name}")
                self.store.delete_collection(name)
                self.refresh_kb_list()
                
        except Exception as e:
            self.logger.error(f"Failed to delete the knowledge base: {str(e)}")
            QMessageBox.critical(self, "Error", f"Failed to delete the knowledge base: {str(e)}")

    def show_model_settings(self):
        """Display the model Settings dialog box"""
        dialog = ModelSettingsDialog(self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            settings = dialog.get_settings()
            # TODO: Application model Settings
            QMessageBox.information(self, "Success", "The model Settings have been updated！")
    ...
    def show_qdrant_settings(self):
        """Display the Qdrant server Settings dialog box"""
        dialog = QDialog(self)
        ...
        # Connection mode: Local storage or server
        ...
        # Initialization is based on the current Settings
    ...
    def save_qdrant_settings(self, mode, host, port, dialog):
        """Save the Settings of the Qdrant server"""
        try:
            ...
            # Read the existing Settings (if any)
            settings = {}
            if os.path.exists(settings_path):
                try:
                    with open(settings_path, "r", encoding="utf-8") as f:
